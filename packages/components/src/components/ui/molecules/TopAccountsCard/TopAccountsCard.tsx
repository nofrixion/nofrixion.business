  ...
  return (
    ...
    >
      <div className={mergedClasses()}>
        <span className="text-sm leading-8 font-semibold">{!isLoading && account.accountName}</span>
        {!isLoading && account && (
...
export const TopAccountsCard: React.FC<TopAccountsCardProps> = ({
  ...
  const totalAvailableBalance = accounts?.reduce((acc, curr) => acc + curr.availableBalance, 0)
    ...
    >
      <div className="flex justify-between items-end mb-10 mt-6">
        ...
        <div className="flex flex-col text-right">
          <span className=" font-normal leading-4 mt-2 mr-1 mb-2 text-[13px] font-inter-fontFeatureSettings">
            Total 3 accounts
          </span>
          {totalAvailableBalance && totalBalance && accounts && currency && (
            <AccountBalance
              size={'medium'}
      ...
      <span className="text-xs font-normal text-grey-text mb-4">MOST ACTIVE ACCOUNTS</span>

      <div className="w-full">
        {/* TODO: Do loading state with skeleton. Map 10 elements and show skeleton */}
        {isLoading &&
          Array.from({ length: 10 }).map((_, i) => (
            <TopAccountRow key={`loading-tx-${i}`} isLoading />
          ))}
        ...
        {!isLoading && (!accounts || accounts.length == 0) && (
          <div className="flex flex-col -mx-6 md:-mx-10">
            ...
            <span className="text-base mx-auto text-center text-grey-text">
              Here you will see your top accounts <br /> from all your accounts.
            </span>
          </div>
        )}
        ...
        {!isLoading &&
          ...
          accounts.length > 0 &&
          accounts.map((acc) => (
            <TopAccountRow key={`latest-transaction-${acc.accountID}`} account={acc} />
          ))}
      </div>
    </Card>
  )
