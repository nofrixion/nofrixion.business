...
import { PaymentRequestStatus } from "@nofrixion/utils/types";
  ...
  return (
    ...
    >
      ...
      <div className="relative items-center xl:flex xl:justify-between w-full">
        <div
          className={cn("animate-pulse absolute left-1/2 top-0 bottom-0 my-auto -translate-x-1/2 flex items-center", {
            invisible: !isLoading,
          })}
        >
        ...
        <div>
          <span
            className={cn("block text-[1.75rem]/6 font-medium truncate leading-6 lg:leading-[48px]", {
              invisible: isLoading,
            })}
          >
            {totalRecords}
        ...
        <div className="hidden xl:flex flex-col justify-center items-end">
          {totalAmountInEuros !== undefined && (
            <span
              className={cn("text-xs lg:text-sm/6 font-medium", {
                invisible: isLoading,
              })}
            >
              € {formatAmount(totalAmountInEuros)}
            </span>
          )}
          {totalAmountInPounds !== undefined && (
            ...
            >
              £ {formatAmount(totalAmountInPounds)}
            </span>
          )}
        </div>
      </div>
