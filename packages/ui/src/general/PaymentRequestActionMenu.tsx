...
const PaymentRequestActionMenuItemContent: React.FC<PaymentRequestActionMenuItemContentProps> = ({
  label,
  iconName,
}) => {
  return (
          ...
          >
            {onDuplicate && (
              <DropdownMenu.Item className={actionItem()} onClick={onDuplicateClick}>
                <PaymentRequestActionMenuItemContent label="Duplicate" iconName="duplicate/16" />
              </DropdownMenu.Item>
            )}
            {onOpenPaymentPage && (
              <DropdownMenu.Item className={actionItem()} onClick={onOpenPaymentPageClick}>
                <PaymentRequestActionMenuItemContent label="Open payment page" iconName="open/16" />
              </DropdownMenu.Item>
            )}
            {onCopyLink && (
              <DropdownMenu.Item className={actionItem()} onClick={onCopyLinkClick}>
                <PaymentRequestActionMenuItemContent label="Copy payment link" iconName="link/16" />
              </DropdownMenu.Item>
            )}
            {onDelete ? (
              <DropdownMenu.Item className={actionItem({ intent: "negative" })} onClick={onDeleteClick}>
                <PaymentRequestActionMenuItemContent label="Delete" iconName="delete/16" />
              </DropdownMenu.Item>
            ) : (
              <DropdownMenu.Item className={actionItem({ intent: "disabled" })} disabled>
                ...
                >
                  {/* TODO: Change colors */}
                  <PaymentRequestActionMenuItemContent label="Delete not available" iconName="delete/16" />
                </InfoTooltip>
              </DropdownMenu.Item>
            )}
          </motion.div>
