import { ApiProps, MerchantProps, Account, ApiResponse, HttpMethod } from "@nofrixion/utils/types";
...
export class AccountsClient extends BaseApiClient {
  ...
  async getAccounts({ merchantId }: MerchantProps): Promise<ApiResponse<Account[]>> {
    const response = await this.httpRequest<Account[]>(`${this.url}/?merchantID=${merchantId}`, HttpMethod.GET);
