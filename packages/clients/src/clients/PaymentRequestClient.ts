...
export class PaymentRequestClient extends BaseApiClient {
   ...
   */
  async get({ paymentRequestId, includeEvents = false }: PaymentRequestProps): Promise<ApiResponse<PaymentRequest>> {
  ...
  async minimal(paymentRequestId: string): Promise<ApiResponse<PaymentRequestMinimal>> {
    return await this.httpRequest<PaymentRequestMinimal>(`${this.apiUrl}/${paymentRequestId}/minimal`, HttpMethod.GET);
  }
  ...
  async delete(paymentRequestId: string): Promise<{
    ...
    return response.status === "success" ? { success: true } : { success: false, error: response.error };
  }
  ...
  async refundCardPayment(
    ...
    const response = await this.httpRequest(`${this.apiUrl}/${paymentRequestId}/card/void/${amount}`, HttpMethod.POST, {
      authorizationID: authorizationId,
    });
