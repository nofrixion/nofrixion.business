...
export class MerchantClient extends BaseApiClient {
  ...
  async getBankSettings({ merchantId }: MerchantProps): Promise<ApiResponse<MerchantBankSettings>> {
    return await this.httpRequest<MerchantBankSettings>(`${this.apiUrl}/${merchantId}/banksettings`, HttpMethod.GET);
  }
  ...
  async deleteTag(
    ...
    const response = await this.httpRequest(`${this.apiUrl}/${merchantId}/tags/${tagId}`, HttpMethod.DELETE);
